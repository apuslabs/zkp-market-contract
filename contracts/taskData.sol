...
contract TaskData {
    ...
    function syncTask(
        ...
        uint256 _uniqID,
        address _assigner,
        ApusData.TaskStatus _stat,
        uint256 _rewardAmount,
        address _rewardRecipient,
        uint256 _assignTime,
        uint256 _proveTime
    ) public {
        (bool exists, ) = isTaskIndexExists(_uniqID, ApusData.TaskType.TaikoZKEvm);
        if (exists) {
            return ;
        }
        setTaskIndex(_uniqID,  ApusData.TaskType.TaikoZKEvm, tasks.length + 1);
        tasks.push(ApusData.Task(tasks.length + 1, _clientId, _uniqID, _assigner, new bytes(0), ApusData.TaskType.TaikoZKEvm, _stat, new bytes(0), ApusData.rewardInfo(_rewardRecipient, _rewardAmount), 1000, _assignTime, _proveTime));
    }
    ...
    function postTask(ApusData.TaskType _tp, uint256 uniqID, bytes calldata input, uint64 expiry, ApusData.rewardInfo memory ri) public {
        ...
        setTaskIndex(uniqID, _tp, tasks.length + 1);
